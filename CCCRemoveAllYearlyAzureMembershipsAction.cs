...
{
    public class CCCRemoveAllYearlyAzureMembershipsAction : IApiProviderFor<QER.CompositionApi.Portal.PortalApiProject>, IApiProvider
    {
        public void Build(IApiBuilder builder)
        {
            builder.AddMethod(Method.Define("webportalplus/removeallyearlyazurememberships/action")
                .Handle<PostedID>("POST", async (posted, qr, ct) =>
                {
                    ...
                    using (var reader = runner.SqlExecute("CCC_DE_YearlyAttestationSubAzureTeamsGroups", new[]
                    {
                    ...
                    foreach (var key in assignmentkeys)
                    {
                        ...
                        var q2 = Query.From("AADUserInGroup").Where(string.Format("XObjectKey = '{0}' and ((XOrigin & 8) = 8)", key)).SelectAll();
